...
#
# CRITICAL: credential_process MUST use absolute paths (especially on macOS)
# - ❌ WRONG: credential_process = ./credential-process.sh
# - ✅ RIGHT: credential_process = $cred_script
#
# macOS is particularly strict about this requirement.
# See: docs/guides/TROUBLESHOOTING.md#credential-process-script-not-found
...
if grep -qE '^\s*credential_process\s*=' "$HOME/.aws/config"; then
    ...
    if grep -E '^\s*credential_process\s*=' "$HOME/.aws/config" | grep -qvE '^\s*credential_process\s*=\s*/'; then
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            echo "  🍎 macOS is particularly strict about this - paths like './credential-process.sh' WILL NOT WORK"
            echo "  You must use the full path like: /Users/$(whoami)/.aws/credential-process.sh"
        fi
        ...
        if [[ "$(uname)" == "Darwin" ]]; then
            echo "  ✓ macOS: Absolute paths confirmed (required for macOS compatibility)"
        fi
    fi
fi
...
if [[ "$SKIP_GPG_CONFIG" != "true" ]] && [[ -f "$gpg_agent_conf" ]]; then
    ...
    if grep -q "^pinentry-program" "$gpg_agent_conf"; then
        ...
        if [[ ! -x "$current_pinentry" ]]; then
            ...
            print_success "Pinentry program already configured correctly"
        fi
    else
        print_info "Adding pinentry program to existing config"
...
# Pinentry program selection (automatically detected for your platform)
